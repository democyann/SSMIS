...
var fileType = '*.txt; *.doc; *.docx; *.xls; *.xlsx; *.ppt; *.pptx; *.jpg; *.gif; *.png; *.pdf;*.rar; *.zip; *.dwg';
var fileDesc = '一般文件';
...
function renderUploader (multi, fileExt, fileDesc, fileSize) {
    if (navigator.userAgent.search(/android|iphone|ipad/i) == -1) {
		...
		'onComplete': function(event, ID, fileObj, response, data) {
			var htmlStr = $("#systop-uploaded-files1").html();
			// icon 图标前缀
			...
			$("#systop-uploaded-files1").html(htmlStr);
			
			if (multi){
				Systop_FileAttch_IDS = $("#fileAttchIds1").val();
				Systop_FileAttch_IDS += (response + ",");
			}else{
				Systop_FileAttch_IDS = response + ",";
			}
			$("#fileAttchIds1").val(Systop_FileAttch_IDS);
		 }
  });
    }else
    {
            $('#systop_upload1').uploadifive({
...
function viewFileAttchList1(fileIds, viewDel,containerId){
	...
	$.ajax({
	    ...
	    success: function(data, textStatus){
            containerId = containerId || "#systop_file_list1";
